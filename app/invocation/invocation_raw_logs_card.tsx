              ...
              {invocation.event
                ...
                .map(event =>
                  <div className="raw-event">
                    <div className="raw-event-title" onClick={this.handleEventClicked.bind(this, event)}>[{this.state.expandedMap.get(event.sequenceNumber) ? "-" : "+"}] Build event {event.sequenceNumber} - {Object.keys(event.buildEvent).filter(key => key != "id" && key != "children").join(", ")}</div>
                    {this.state.expandedMap.get(event.sequenceNumber) && <div>{JSON.stringify((event.buildEvent as any).toJSON(), null, 4)}</div>}
                  </div>
                )}
            </div>
          )}
        </div>
        {this.props.pageSize && this.props.model.invocations.flatMap(invocation => invocation.event).length > (this.props.pageSize * this.state.numPages) && !!this.state.numPages &&
          <div className="more" onClick={this.handleMoreClicked.bind(this)}>See more events</div>}
      </div>
    </div>
  }
}
